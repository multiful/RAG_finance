# ======================================================================
# FSC Policy RAG System | 모듈: app.services.rag_service
# 최종 수정일: 2026-04-07
...
3. "summary"에는 질문에 대한 한두 문장의 직접 답을 먼저 쓰고, "answer"에서는 근거·세부·인용을 보강하십시오 (요약은 질문과 의미적으로 동일한 방향이어야 함).
4. {mode_instruction}
5. 문서에 없는 내용은 절대 추측하여 답변하지 마십시오. 모르는 경우 '제공된 문서에서 관련 내용을 찾을 수 없습니다'라고 하십시오.
6. 금융·규제 용어를 정확히 쓰고, 보도자료·고시·지침·시행령 등 문서 유형에 맞는 어조를 유지하십시오.
7. 참고 문서 제목·본문에 나온 제도명·고시명·조문 표기가 있으면 답변에서도 동일하게 사용하십시오. 시행일·게시일·적용 시점이 문서에 있으면 함께 언급하십시오.
8. 비율·한도·기한·의무·적용 대상 등 수치·조건은 해당 문서에 근거가 있을 때만 서술하고, 반드시 [번호] 인용과 연결하십시오.
9. "answer"는 불필요한 반복을 피하고 핵심 위주로 2000자 이내로 쓰세요(인용은 유지).
10. "answer" 문자열 안에는 반드시 참고 문서 번호 [1]~[{len(chunks)}] 형식의 인용을 문장마다 포함하십시오. 인용 없이 문장을 쓰지 마십시오.
11. 반드시 아래 JSON 형식으로만 출력하십시오.

출력 JSON 형식:
{{
    "answer": "상세 답변 내용 (문장별 [1][2] 인용 필수)",
    "summary": "질문에 대한 직접 답을 포함한 3줄 이내 요약",
    "industry_impact": {{
        "BANKING": 0.0~1.0,
        "INSURANCE": 0.0~1.0,
        "SECURITIES": 0.0~1.0
